        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-11 pr-4 py-3 bg-slate-800/50 border border-white/10 rounded-xl text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-colors"
            />
          </div>
          <select 
            value={filterStatus}
            onChange={(e) => setFilterStatus(e.target.value as any)}
            className="px-5 py-3 bg-slate-800/50 border border-white/10 text-white rounded-xl hover:bg-white/5 transition-colors outline-none focus:ring-2 focus:ring-indigo-500/50"
          >
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredListings.map((listing) => (
              <div key={listing.id} className="bg-slate-800/50 border border-white/10 rounded-2xl overflow-hidden hover:border-indigo-500/30 transition-colors group flex flex-col">
                {/* Property Image */}
                <div className="relative h-56 overflow-hidden shrink-0">
                  ...
                  <div className="absolute top-4 left-4">
                    <span className={`px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider border shadow-lg ${getStatusStyles(listing.status)}`}>
                      {listing.status}
                    </span>
                  </div>
                  ...
                  <div className="absolute top-4 right-4 flex gap-2 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300">
                    <button className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center text-white hover:bg-white/20 transition-colors shadow-lg shadow-black/20 border border-white/10">
                      <Edit2 size={16} />
                    </button>
                    <button className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center text-white hover:bg-white/20 transition-colors shadow-lg shadow-black/20 border border-white/10">
                      <Share2 size={16} />
                    ...
                    <button 
                      ...
                      className="w-9 h-9 rounded-full bg-rose-500/90 flex items-center justify-center text-white hover:bg-rose-600 transition-colors shadow-lg shadow-black/20 border border-rose-500/20"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
          ...
          <div className="flex flex-col items-center justify-center py-32 text-center bg-slate-800/30 border border-white/5 rounded-3xl">
